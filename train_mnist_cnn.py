...
# MNISTデータをロード
print('load temperature dataset')
import pickle
#f = open('split.dat', 'rb')
mnist = pickle.load(open('split.dat', 'r''b'), encoding = 'bytes')
#f.close()
...
X_train = X_train.reshape((len(X_train), 1, 110, 47))
X_test = X_test.reshape((len(X_test), 1, 110, 47))
...
model = chainer.FunctionSet(conv1=F.Convolution2D(1, 20, 5),   # 入力1枚、出力20枚、フィルタサイズ5ピクセル
                            ...
                            l1=F.Linear(11250, 500),             # 入力800ユニット、出力500ユニット
...
for epoch in range(1, n_epoch + 1):
    ...
    print('test accuracy', sum_accuracy / N_test)
